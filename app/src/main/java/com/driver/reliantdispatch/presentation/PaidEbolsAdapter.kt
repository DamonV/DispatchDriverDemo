...
import com.driver.reliantdispatch.databinding.ViewEbolPaidRowBinding
import com.driver.reliantdispatch.domain.entities.joined.EbolJoined


class PaidEbolsAdapter(
    val mViewModel: PaidViewModel,
    val mActivity: FragmentActivity
): PagedListAdapter<EbolJoined, PaidEbolsAdapter.ViewHolder>(diffCallback){

    class ViewHolder(val binding: ViewEbolPaidRowBinding): RecyclerView.ViewHolder(binding.root){
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ViewEbolPaidRowBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    fun onArchiveClick(v: View, item: EbolJoined, pos: Int){
        val builder = AlertDialog.Builder(v.context,  R.style.AlertDialogTheme)
        builder.setTitle(R.string.dialog_title_warning)
            .setMessage(v.resources.getString(R.string.archive_ebol_confirm))
            .setPositiveButton(
                android.R.string.ok
            ) { dialog, _ ->
                dialog.cancel()
                mViewModel.onArchiveClick(item, pos)
            }
            .setNegativeButton(

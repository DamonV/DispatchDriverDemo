...
import com.driver.reliantdispatch.databinding.ViewEbolDeliveredRowBinding
import com.driver.reliantdispatch.domain.entities.joined.EbolJoined


class DeliveredEbolsAdapter(
    val mViewModel: DeliveredViewModel,
    val mActivity: FragmentActivity
): PagedListAdapter<EbolJoined, DeliveredEbolsAdapter.ViewHolder>(diffCallback){

    class ViewHolder(val binding: ViewEbolDeliveredRowBinding): RecyclerView.ViewHolder(binding.root){
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ViewEbolDeliveredRowBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    fun onPaidClick(v: View, item: EbolJoined){
        val amount = item.ebol?.paymentAmount ?: "-"
        val builder = AlertDialog.Builder(v.context,  R.style.AlertDialogTheme)
        builder.setTitle(R.string.dialog_title_warning)
            .setMessage(v.resources.getString(R.string.text_submit_paid, amount))
            .setPositiveButton(
                android.R.string.ok
            ) { dialog, _ ->
                dialog.cancel()
                mViewModel.onPaidClick(item.ebol?.id)
            }
            .setNegativeButton(
